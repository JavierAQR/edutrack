...
import type { StudentInSectionResponse, StudentWithAverageResponse } from '../../types';
      ...
      const fetchData = async () => {
        try {
          const [studentsData, averagesData] = await Promise.all([
            ...
            axios.get(`http://localhost:8080/api/sections/${sectionId}/students-averages`),
          ]);
            ...
            <tbody>
              {students.map(student => (
                <tr key={student.id} className="border-b">
                  <td className="px-4 py-2">{student.name} {student.lastname}</td>
